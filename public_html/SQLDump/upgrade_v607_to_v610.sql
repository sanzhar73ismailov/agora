CREATE TABLE IF NOT EXISTS `probid_bulk_listings` (
...
ALTER TABLE `probid_auctions` ADD `bulk_list` TINYINT NOT NULL ;
ALTER TABLE `probid_auctions` ADD INDEX `bulk_pending` ( `owner_id` , `bulk_list` ) ;

ALTER TABLE `probid_gen_setts` ADD `email_admin_title` VARCHAR( 100 ) NOT NULL DEFAULT 'Site Administrator',
ADD `browse_thumb_size` INT NOT NULL DEFAULT '50' ;

ALTER TABLE `probid_gen_setts` ADD `display_free_fees` TINYINT NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `enable_proxy_bidding` TINYINT NOT NULL DEFAULT '1' ;

ALTER TABLE `probid_users` ADD `enable_item_counter` TINYINT NOT NULL DEFAULT '1' ;

ALTER TABLE `probid_gen_setts` ADD `free_category_change` TINYINT NOT NULL ;

ALTER TABLE `probid_auction_durations` ADD `order_id` INT NOT NULL ,
ADD INDEX ( `order_id` ) ;

ALTER TABLE `probid_layout_setts` ADD `enable_site_fees_page` TINYINT NOT NULL DEFAULT '1' ;

ALTER TABLE `probid_auctions` ADD `bulk_id` INT NOT NULL ;
ALTER TABLE `probid_auctions` ADD INDEX ( `bulk_id` ) ;


-- ALTER TABLE `probid_gen_setts` ADD `max_nb_bids` INT NOT NULL ;

-- ALTER TABLE `probid_bids` ADD `is_bid` TINYINT NOT NULL ;
-- ALTER TABLE `probid_bids` ADD INDEX ( `auction_id` , `bidder_id` , `is_bid` ) ;

ALTER TABLE `probid_fees` ADD `swap_fee_calc_type` ENUM( 'flat', 'percent' ) NOT NULL DEFAULT 'flat' ;

ALTER TABLE `probid_users` ADD `store_password` VARCHAR( 100 ) NOT NULL ;
ALTER TABLE `probid_categories` ADD `cat_password` VARCHAR( 255 ) NOT NULL ;

ALTER TABLE `probid_auctions` ADD `disable_sniping` TINYINT NOT NULL ;

ALTER TABLE `probid_users` ADD `show_makeoffer_ranges` TINYINT NOT NULL DEFAULT '1';

ALTER TABLE `probid_fees` ADD `endauction_calc_type` TINYINT NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `csv_delimiter` ENUM( 'comma', 'semicolon' ) NOT NULL DEFAULT 'comma' ;

ALTER TABLE `probid_users` CHANGE `pc_postage_calc_type` `pc_postage_calc_type` ENUM( 'default', 'custom', 'carriers' ) NOT NULL ;

CREATE TABLE IF NOT EXISTS `probid_shipping_carriers` (
...
INSERT INTO `probid_shipping_carriers` (`carrier_id`, `name`, `enabled`, `weight_unit`, `logo_url`) VALUES
(3, 'USPS', 0, 'pounds', 'img/usps_logo.gif'),
(4, 'FedEx', 0, 'lbs', 'img/fedex_logo.gif'),
(5, 'UPS', 0, 'lbs', 'img/ups_logo.gif');

ALTER TABLE `probid_users` ADD `shipping_carriers` VARCHAR( 255 ) NOT NULL ;

ALTER TABLE `probid_winners` ADD `carrier_id` INT NOT NULL ,
ADD `shipping_method` VARCHAR( 255 ) NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `carrier_setts` TEXT NOT NULL ;
...
INSERT INTO `probid_countries` (`id`, `name`, `country_order`, `parent_id`, `country_iso_code`) VALUES
...
ALTER TABLE `probid_users` ADD `store_expiration_email` TINYINT NOT NULL ,
ADD INDEX ( `store_expiration_email` ) ;

CREATE TABLE `probid_bids_retracted` (
...
ALTER TABLE `probid_bids_retracted` ADD INDEX ( `retraction_date` ) ;

ALTER TABLE `probid_gen_setts` ADD `limit_nb_bids` TINYINT NOT NULL ;

ALTER TABLE `probid_users` ADD `limit_offers` INT NOT NULL ,
ADD `limit_bids` INT NOT NULL ;

ALTER TABLE `probid_auction_durations` ADD `selected` TINYINT NOT NULL ;

ALTER TABLE `probid_fees` ADD `durations_fee` TEXT NOT NULL ;

CREATE TABLE `probid_saved_searches` (
...
ALTER TABLE `probid_saved_searches` ADD INDEX ( `user_id` , `reg_date` ) ;

ALTER TABLE `probid_users` ADD `suspension_date` INT NOT NULL ,
ADD INDEX ( `suspension_date` ) ;

ALTER TABLE `probid_gen_setts` ADD `suspension_date_days` INT NOT NULL ;

ALTER TABLE `probid_vouchers` ADD `user_id` INT NOT NULL ,
ADD INDEX ( `user_id` ) ;

ALTER TABLE `probid_auctions` CHANGE `item_weight` `item_weight` DOUBLE( 16, 2 ) NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `seller_verification_refund` TINYINT NOT NULL ;

ALTER TABLE `probid_users` ADD `seller_tax_amount` DOUBLE( 16, 2 ) NOT NULL ;

ALTER TABLE `probid_users` ADD `show_watch_list` TINYINT NOT NULL DEFAULT '1' ;

ALTER TABLE `probid_saved_searches` ADD `name` VARCHAR( 255 ) NOT NULL ;


ALTER TABLE `probid_categories` ADD `enable_auctions` TINYINT NOT NULL DEFAULT '1',
ADD `enable_wanted` TINYINT NOT NULL DEFAULT '1';

ALTER TABLE `probid_categories` ADD INDEX ( `enable_auctions` ) ; 
ALTER TABLE `probid_categories` ADD INDEX ( `enable_wanted` ) ;

UPDATE `probid_categories` SET enable_auctions =1,
enable_wanted =1 WHERE parent_id =0 ;

INSERT INTO `probid_payment_gateways` 
(`pg_id` ,`name` ,`checked` ,`dp_enabled` ,`logo_url` )
VALUES 
(NULL , 'GUNPAL', '0', '0', 'img/gunpal_logo.gif');

ALTER TABLE `probid_gen_setts` ADD `pg_gunpal_id` VARCHAR( 255 ) NOT NULL ;

ALTER TABLE `probid_users` ADD `pg_gunpal_id` VARCHAR( 255 ) NOT NULL ;

ALTER TABLE `probid_auctions` ADD `seller_ip` VARCHAR( 20 ) NOT NULL ;

ALTER TABLE `probid_fees` ADD `bidder_verification_fee` DOUBLE( 16, 2 ) NOT NULL ,
ADD `bidder_verification_recurring` INT NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `enable_bidder_verification` TINYINT NOT NULL ,
ADD `bidder_verification_mandatory` INT NOT NULL ;

ALTER TABLE `probid_users` ADD `bidder_verified` TINYINT NOT NULL ,
ADD `bidder_verif_last_payment` INT NOT NULL ,
ADD `bidder_verif_next_payment` INT NOT NULL ;

ALTER TABLE `probid_gen_setts` ADD `hide_empty_stores` TINYINT NOT NULL ;

ALTER TABLE `probid_users` ADD `is_vacation` TINYINT NOT NULL ;

UPDATE `probid_currencies` SET `symbol`='RUB' WHERE `symbol`='RUR' ;